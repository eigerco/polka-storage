...
| ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------- | ----------- |
| `WPoStProvingPeriod`           | The average period for proving all sectors maintained by a storage provider.                                                        | 5 Minutes   |
...
| `WPoStPeriodDeadlines`         | Represents how many challenge deadline there are in 1 proving period. Closely tied to `WPoStChallengeWindow`.                       | 48          |
| `MinSectorExpiration`          | Minimum number of blocks past the current block a sector may be set to expire.                                                      | 5 Minutes   |
| `MaxSectorExpirationExtension` | Maximum number of blocks past the current block a sector may be set to expire.                                                      | 60 Minutes  |
| `SectorMaximumLifetime`        | Maximum number of blocks a sector can stay in pre-committed state                                                                   | 120 Minutes |
| `MaxProveCommitDuration`       | Maximum duration to allow for the sealing process for seal algorithms.                                                              | 5 Minutes   |
| `MaxPartitionsPerDeadline`     | Maximum amount of partition that can be assigned to a single deadline.                                                              | 3000        |
| `FaultMaxAge`                  | Maximum time period a fault can exist before being removed by the system,                                                           | 210 Minutes |
| `FaultDeclarationCutoff`       | Amount of time before a deadline opens that a storage provider can declare or recover a fault.                                      | 2 Minutes   |
