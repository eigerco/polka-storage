...
| `TimeUnitInBlocks` | How many blocks are created in a time unit.                                   | 5 blocks = 1 minute    |
| `MinDealDuration`  | Minimum minutes[^time_unit] an activated deal should last.                                   | 5     |
| `MaxDealDuration`  | Maximum minutes[^time_unit] an activated deal should last.                                   | 180   |

[^time_unit]: `MinDealDuration` and `MaxDealDuration` are both calculated in terms of `TimeUnitInBlocks`, as such, if `TimeUnitInBlocks` is equivalent to 1 minute (the amount in the demo configuration), a value of 5 `MinDealDuration` is equivalent to 5 minutes.
