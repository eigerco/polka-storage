...
use crate::{
    ...
    deals::{ClientDealProposalOf, DealProposalOf, DealState},
    ...
    T: frame_system::Config<AccountId = AccountId32>,
{
        ...
        BlockNumberFor<T>: From<u64>,
    {
        ...
        BenchmarkData {
            ...
            timeline: {
                let _proving_period_offset = 15u64;
                let _proving_period_start_initial = 75u64;
                let _seal_randomness_height = 93u64;
                let _sector_expiration = 230u64;
                let _interactive_block_number = 110u64;
                let _prove_commit_sectors = 111u64;
                let _deadline_index = 0u64;
                let _deadline_challenge_block = 245u64;
                let _deadline_start = 255u64;
                let _submit_windowed_post = 256u64;
                let _deadline_close = 275u64;
                SectorTimeline::new(
                    AccountId32::new([
                    ...
                    Absolute::from(BlockNumberFor::<T>::from(1u64)),
                    Absolute::from(BlockNumberFor::<T>::from(5u64)),
                    Absolute::from(BlockNumberFor::<T>::from(100u64)),
                    vec![DealTimeline::new(
                        Absolute::from(BlockNumberFor::<T>::from(180u64)),
                        Relative::from(BlockNumberFor::<T>::from(50u64)),
                    )],
                    3u64,
                    Relative::from(BlockNumberFor::<T>::from(60u64)),
                    Relative::from(BlockNumberFor::<T>::from(20u64)),
                    Relative::from(BlockNumberFor::<T>::from(10u64)),
                    Relative::from(BlockNumberFor::<T>::from(10u64)),
                )
