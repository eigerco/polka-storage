...
impl UnsealedSector {
    /// Returns the sector expiration value, calculated by getting the maximum expiration block
    /// of the sector deals; if no deals exist (i.e. the sector was just created) returns `None`.
    fn sector_expiration(&self) -> Option<u64> {
        self.deals.iter().map(|(_, deal)| deal.end_block).max()
    }
