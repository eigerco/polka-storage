...
async fn submit_windowed_post(
    ...
    let Some(deadline_state) = state.xt_client.deadline_state(&state.xt_keypair.account_id().into(), deadline_index).await? else {
    ...
    if deadline_state.partitions.0.len() > 1 {
        todo!("I don't know what to do: polka-storage#595");
    }

    // TODO(@th7nder,02/12/2024): bruh, because of this BoundedBTreeMap serialization magic hell is going on here.
    // basically get all partition numbers and their sectors from magical data structures generated by macros
    let partitions = deadline_state.partitions.0.iter().map(|(k, _v)| k).cloned().collect();
    let sectors = deadline_state.partitions.0[0].1.sectors.0.clone();
    ...
    tracing::info!("Proving PoSt partitions... {:?}", deadline_state.partitions);
