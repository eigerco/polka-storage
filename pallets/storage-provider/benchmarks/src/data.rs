...
use primitives::{
    ...
    MAX_LABEL_SIZE, MAX_PROOFS_PER_BLOCK, MAX_SEAL_PROOF_BYTES, MAX_SECTORS_PER_CALL,
    PEER_ID_MAX_BYTES,
};
use sp_core::{ConstU32, Get};
use sp_runtime::{AccountId32, BoundedVec, MultiSigner};

use crate::{
    accounts::generate_benchmark_account,
...
where
    T: crate::Config<PeerId = BoundedVec<u8, ConstU32<PEER_ID_MAX_BYTES>>, AccountId = AccountId32>,
    BlockNumberFor<T>: From<u64>,
{
    pub fn provider(&self) -> StorageProvider<T> {
        let (account_id, sign) = generate_benchmark_account(&self.provider_name);

        let peer_id: &[u8; 32] = &account_id.as_ref();
        let peer_id: T::PeerId = peer_id.to_vec().try_into().unwrap();
        StorageProvider {
            account_id,
            sign,
            peer_id,
    ...
    pub fn deal_proposals(
        ...
        self.sectors
            .iter()
            .take(limit as usize)
            .map(|sector| sector.deal_proposal::<T>(self, client.clone()))
            .collect::<Vec<_>>()
            .try_into()
            .unwrap()
    }
    ...
    pub fn pre_commit_sectors(
        ...
        self.sectors
            .iter()
            .take(limit as usize)
            .map(|sector| sector.pre_commit_sector::<T>(&self))
            .collect::<Vec<_>>()
            .try_into()
            .unwrap()
    }
    ...
    pub fn prove_commit_sectors(
        ...
        self.sectors
            .iter()
            .take(limit as usize)
            .map(|sector| sector.prove_commit_sector::<T>())
            .collect::<Vec<_>>()
            .try_into()
            .unwrap()
    }
...
impl SectorData {
    ...
    where
        T: crate::Config<
            PeerId = BoundedVec<u8, ConstU32<PEER_ID_MAX_BYTES>>,
            AccountId = AccountId32,
        >,
        BlockNumberFor<T>: From<u64>,
    {
        let min_dur = T::MinDealDuration::get();
        let label = vec![u32::from(self.sector_number) as u8; MAX_LABEL_SIZE as usize];
        let proposal = DealProposalOf::<T> {
            piece_cid: benchmark_data
                .comm_p
                .cid()
                .to_bytes()
                .try_into()
                .expect("hash is always 32 bytes"),
            piece_size: *self.padded_piece_size,
            client: client.0,
            provider: benchmark_data.provider().account_id,
        ...
        sign_proposal::<T>(client.1, proposal)
    }

    pub fn pre_commit_sector<T>(
        &self,
        benchmark_data: &BenchmarkData<T>,
    ...
    {
        SectorPreCommitInfo {
            seal_proof: benchmark_data.seal_proof,
            sector_number: self.sector_number,
            sealed_cid: self.comm_r.cid().to_bytes().try_into().unwrap(),
            deal_ids: vec![u64::from(self.sector_number) as u64]
                .try_into()
                .unwrap(),
            expiration: min(
            ...
            unsealed_cid: self.comm_d.cid().to_bytes().try_into().unwrap(),
            seal_randomness_height: benchmark_data.seal_randomness_height.into(),
        }
    }

    pub fn prove_commit_sector<T>(&self) -> ProveCommitSector
    where
        T: crate::Config,
    {
        // TODO(@Jinxit,#790,28/02/2025): This needs to be updated when we add multiple proofs to the same file.
        let proof: BoundedVec<u8, ConstU32<MAX_SEAL_PROOF_BYTES>> =
            self.proof.to_vec().try_into().unwrap();
        let proofs: BoundedVec<_, ConstU32<MAX_PROOFS_PER_BLOCK>> = vec![proof].try_into().unwrap();
        ProveCommitSector {
            sector_number: self.sector_number,
            proofs,
        }
...
pub fn get_benchmark_data<T>(sector_size: SectorSize) -> BenchmarkData<T> {
    match sector_size {
        // DO NOT MODIFY
        ...
        SectorSize::_2KiB => BenchmarkData {
            ...
            comm_p: Commitment::<CommP>::from_cid(
                &Cid::from_str("baga6ea4seaqbfhdvmk5qygevit25ztjwl7voyikb5k2fqcl2lsuefhaqtukuiii")
                    .expect("valid cid"),
            )
