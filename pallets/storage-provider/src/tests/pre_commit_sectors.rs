...
use crate::{
    ...
    tests::{
        account, events, publish_deals, register_storage_provider, run_to_block,
...
fn successfully_precommited_no_deals() {
    new_test_ext().execute_with(|| {
        ...
        Balances::make_free_balance_be(&account(storage_provider), MARKET_BALANCE.into());
        ...
        assert_eq!(
            ...
            1
        ); // Single pre-commit = price is 1
        assert_eq!(Balances::free_balance(&account(storage_provider)), 999);
    });
}
