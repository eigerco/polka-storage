...
fn successfully_precommited() {
    new_test_ext().execute_with(|| {
        ...
        StorageProvider::pre_commit_sector_batch(
...
fn fails_should_be_signed() {
    new_test_ext().execute_with(|| {
        ...
        // Run pre commit extrinsic
        assert_noop!(
            StorageProvider::pre_commit_sector_batch(RuntimeOrigin::none(), bounded_vec![sector]),
...
fn fails_sector_number_already_used() {
    new_test_ext().execute_with(|| {
        ...
        // Run pre commit extrinsic
        assert_ok!(StorageProvider::pre_commit_sector_batch(
            RuntimeOrigin::signed(account(storage_provider)),
            bounded_vec![sector.clone()]
        ));
