...
impl<BlockNumber> Relative<BlockNumber> {
    ...
    pub fn div_ceil(self, rhs: Self) -> <Self as Div<Self>>::Output
    where
        BlockNumber: Zero
            + One
            + TryInto<u32>
            + PartialOrd
            + Add<BlockNumber, Output = BlockNumber>
            + Sub<BlockNumber, Output = BlockNumber>
            + Div<BlockNumber, Output = BlockNumber>,
    {
        assert!(!rhs.0.is_zero());
        if self.0.is_zero() {
            self / rhs
        } else {
            (self - Relative::one()) / rhs + 1
        }
    }
}
...
where
    BlockNumber: Div<BlockNumber, Output = BlockNumber> + TryInto<u32> + Zero,
{
    type Output = u32;
    ...
    fn div(self, rhs: Relative<BlockNumber>) -> Self::Output {
        ...
        (self.0 / rhs.0)
            .try_into()
            .unwrap_or_else(|_| panic!("block number to fit in a u32"))
    }
}
